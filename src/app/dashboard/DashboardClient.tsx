...
function AstroModal({ open, title, desc, type, onClose, onOk, okText, cancelText }: any){
  ...
  return(
    <div style={{position:"fixed", inset:0, zIndex:99999, background:"rgba(20,10,40,.5)", display:"flex", alignItems:"center", justifyContent:"center", padding:"16px", backdropFilter:"blur(6px)"}}>
      <div style={{background:"#fff", borderRadius:"20px", padding:"22px", width:"100%", maxWidth:"360px", boxShadow:"0 20px 40px rgba(0,0,0,.25)", textAlign:"center"}}>
...
export default function DashboardClient({ children, nomeOng }: { children: React.ReactNode, nomeOng: string }) {
  ...
  return (
    <div className={styles.container}>
      ...
      <AstroModal {...modal} onClose={()=>setModal({open:false})} />
    </div>
  );
}